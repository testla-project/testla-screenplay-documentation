...
In this section it is described how the different components (Ability, Action, Task and Question) technically play together during the execution flow.

We broke it down based on weather an Ability got aliased or not.
...
The flow for Tasks is identical.

### Triggering of questions

Questions per se follow the same flow as actions/tasks.
The only difference are the names of internal methods.
...
The lookup of tha desired Ability including the alias information is the same as described above in the Actions section.
