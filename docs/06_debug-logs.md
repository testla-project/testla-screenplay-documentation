...
Testla comes with logging which helps you to debug your test code. When logging is enabled all activities an actor triggers are logged in a comprehensive way to stdout.
...
![Logging example](./img/logging_example.png)
...
![Logging explained](./img/logging_core_explained.png)
...
* Initializer methods parameters
* Any subsequent configuration methods for Actions and Tasks
* Check methods for Questions

But no worries, Testla Screenplay Core offers tools to enhance the logging  to have the above information gap in the logging output.
...
Actions, Tasks and Questions might have subsequent methods calls to further enhance the activity. An example is ability aliasing (which we will explain in a later section). To announce those kind of information to the logger use `addToCallStack`. This method is available to all above mentioned entities out of the box.

```typescript
public withAbilityAlias(alias: string | undefined) {
    this.abilityAlias = alias;

    // if alias provided, add information to logging
    if (alias) {
...
In some cases it is not possible to listen to the event emitter i.e. with a setup of main and worker processes - an example for that is playwright. For these cases testla can write structured log events to stdout which can be parsed by your custom integration.
